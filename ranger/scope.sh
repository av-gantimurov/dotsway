    ...
    case "${mimetype}" in
        ...
        *ms-excel)
        ...
        application/vnd.microsoft.portable-executable)
            # env
            emit "${$FILE_PATH}" | pemeta --tabular && exit 5
            ;;
        ...
        application/octet-stream)
            SIZE=$(stat --dereference --format='%s' "$FILE_PATH")
            if [ 1024 -lt "$SIZE" ]; then
                echo "Hexdump only first 1024 of $SIZE"
            else
                echo "Hexdump full file"
            fi
            hexdump --canonical --length 1024 "$FILE_PATH" && exit 5
            exit 1;;
    esac
}

handle_fallback() {
    # hexdump -n300 -C "$FILE_PATH"
    echo 'Fallback'
    echo '-- EXIFTOOL --' && exiftool "${FILE_PATH}" && exit 5
